...
pub const VERSION: RuntimeVersion = RuntimeVersion {
	spec_name: create_runtime_str!("substrate"),
	impl_name: create_runtime_str!("substrate-substrate"),
...
//基本模块
		...
		//这是一个联合命名系统，它允许从指定来源添加多个注册服务商。 注册服务商是通过收取一定费用来提供身份验证服务的。 任何人只要缴纳固定押金即可申请新ID，随后可提请任意数量的注册服务商来审核申请(每个服务商均收一次审核费)。 注册服务商的决议是以枚举类型的形式给出的，系统允许复杂的、多层级的决议。
		Identity: pallet_identity::{Pallet, Call, Storage, Event<T>},
		//Society 模块是一种旨在激励用户参与并维护会员制社区的经济游戏。
		//Society: pallet_society::{Pallet, Call, Storage, Event<T>, Config<T>},
		//Recovery pallet是一种基于M-of-N朋友社交的账户恢复工具。如果用户私钥丢失或其他身份验证机制失效了，用户可用它恢复帐户访问权限。 通过这个pallet，用户可用新账户替代旧账户身份做相关函数调用，前提是该旧账户已完成遗失账户恢复流程。 恢复流程是受到账户此前选中的朋友信任关系保护的。 需要得到 N 个朋友当中的最低门鉴数 (M) 个人的同意，才能把原有账户的访问权限授予给另一个账户。
		Recovery: pallet_recovery::{Pallet, Call, Storage, Event<T>},
		//一个简单的模块，它提供了将线性释放曲线放置在帐户锁定余额上的手段。 此模块确保足额锁定，以防止余额因交易费用支付以外的任何原因而降至非锁定金额以下。
		Vesting: pallet_vesting::{Pallet, Call, Storage, Event<T>, Config<T>},
		//利用此模块可实现在指定区块号或指定周期的计划函数调用。 这些提前设定的可调用函数，可以包含调用者名称或匿名，也可跟着被取消。
		Scheduler: pallet_scheduler::{Pallet, Call, Storage, Event<T>},
		//这个模块允许一个帐户授权给其它账户，并通过签名允许授权账户调用其可调用函数。
		Proxy: pallet_proxy::{Pallet, Call, Storage, Event<T>},
		//Multisig 模块是一个用于调用多重签名函数的模块。
		Multisig: pallet_multisig::{Pallet, Call, Storage, Event<T>},
		//Assets 模块是一个用于处理可替代资产，简单而安全的模块。
...
///
/// When you change this, you **MUST** modify [`sign`] in `bin/substrate/testing/src/keyring.rs`!
///
