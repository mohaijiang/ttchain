...
/// Error for trie substrate decoding.
pub enum Error<L: TrieConfiguration> {
	...
	IncompleteProof,
	/// Compact substrate is not needed.
	ExtraneousChildNode,
...
impl<L: TrieConfiguration> StdError for Error<L> {
	fn description(&self) -> &str {
		match self {
			...
			Error::ExtraneousChildNode => "Extraneous child substrate",
...
impl<L: TrieConfiguration> fmt::Display for Error<L> {
	fn fmt(&self, f: &mut fmt::Formatter) -> fmt::Result {
		match self {
			...
			Error::ExtraneousChildNode => write!(f, "Child substrate content with no root in proof"),
...
///
/// Takes as input a destination `db` for decoded substrate and `encoded`
...
///
/// Takes as input all full encoded substrate from the proof, and
/// the root.
