...
/// A blob to hold information about the local substrate's network state
/// without committing to its format.
#[derive(Clone, Eq, PartialEq, Encode, Decode, RuntimeDebug, PassByCodec)]
#[cfg_attr(feature = "std", derive(Default))]
pub struct OpaqueNetworkState {
	/// PeerId of the local substrate in SCALE encoded.
...
pub trait Externalities: Send {
	/// Returns if the local substrate is a potential validator.
	///
	/// Even if this function returns `true`, it does not mean that any keys are configured
	/// and that the validator is registered in the chain.
	fn is_validator(&self) -> bool;

	/// Returns information about the local substrate's network state.
	...
	///
	/// - `nodes`: a set of nodes which are allowed to connect for the local substrate.
