...
//! a fixed interface between a Substrate runtime and a Substrate substrate. For a native runtime the
...
//! The FFI function definition is used when calling from the wasm runtime into the substrate.
//!
...
//! how a type is passed between the wasm runtime and the substrate. Each strategy also provides a derive
//! macro to simplify the implementation.
//!
//! # Performance
//!
//! To not waste any more performance when calling into the substrate, not all types are SCALE encoded
//! when being passed as arguments between the wasm runtime and the substrate. For most types that
...
/// substrate. This interface is callable from a native and a wasm runtime. The macro will generate the
