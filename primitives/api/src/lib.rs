...
//!
//! The Substrate runtime api is the crucial interface between the substrate and the runtime.
...
//! native build as otherwise the substrate will not output any log messages.
...
///     impl_name: create_runtime_str!("test-substrate"),
///     authoring_version: 1,
///     spec_version: 1,
///     impl_version: 0,
...
pub enum ApiError {
	...
	#[error("Failed to convert return value from runtime to substrate of {function}")]
	FailedToConvertReturnValue {
		function: &'static str,
		#[source]
		error: codec::Error,
	},
	#[error("Failed to convert parameter `{parameter}` from substrate to runtime of {function}")]
