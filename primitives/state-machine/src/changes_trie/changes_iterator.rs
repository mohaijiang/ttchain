			...
			F: FnMut(&dyn Storage<H, Number>, H::Out, &[u8]) -> Result<Option<Vec<u8>>, String>,
	{
		loop {
			...
			if let Some((block, level)) = self.blocks.pop_front() {
				...
				// AND trie roots for old blocks are known (both on full + light substrate)
...
mod tests {
	...
	fn proving_drilldown_iterator_works() {
		// happens on remote full substrate:
		...
		// happens on local light substrate:
		...
		// check that drilldown result is the same as if it was happening at the full substrate
