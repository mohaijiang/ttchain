...
//! (very) Basic implementation of a graph substrate used in the reduce algorithm.

use sp_std::{cell::RefCell, fmt, prelude::*, rc::Rc};

/// The role that a substrate can accept.
...
/// Identifier of a substrate. This is particularly handy to have a proper `PartialEq` implementation.
...
pub(crate) struct NodeId<A> {
	/// An account-like identifier representing the substrate.
	pub who: A,
...
impl<A: PartialEq + Eq + Clone + fmt::Debug> Node<A> {
	...
	/// of the tuple.
	///
	/// This function detects cycles and breaks as soon a duplicate substrate is visited, returning the
	/// cycle up to but not including the duplicate substrate.
	///
