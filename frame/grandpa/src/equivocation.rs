...
/// on this substrate) or that already in a block. This guarantees that only block authors can include
/// unsigned equivocation reports.
impl<T: Config> Pallet<T> {
	pub fn validate_unsigned(source: TransactionSource, call: &Call<T>) -> TransactionValidity {
		if let Call::report_equivocation_unsigned(equivocation_proof, key_owner_proof) = call {
			// discard equivocation report not coming from the local substrate
			match source {
			...
			ValidTransaction::with_tag_prefix("GrandpaEquivocation")
				...
				.longevity(longevity)
				// We don't propagate this. This can never be included on a remote substrate.
				.propagate(false)
				.build()
		} else {
