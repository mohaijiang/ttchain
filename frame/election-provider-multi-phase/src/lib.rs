...
//! phase, only validator nodes can submit solutions. A validator substrate who has offchain workers
...
pub mod pallet {
	...
	impl<T: Config> Pallet<T> {
		...
		/// validated when submitted to the pool from the **local** substrate. Effectively, this means
		...
		fn validate_unsigned(source: TransactionSource, call: &Self::Call) -> TransactionValidity {
			if let Call::submit_unsigned(solution, _) = call {
				...
				ValidTransaction::with_tag_prefix("OffchainElection")
					...
					// We don't propagate this. This can never be validated at a remote substrate.
					.propagate(false)
					.build()
			} else {
