...
pub trait LeafDataProvider {
	...
	/// in the leaf substrate appended MMR at this block.
	///
...
impl<H: traits::Hash, L: FullLeaf> DataOrHash<H, L> {
	...
	///
	/// Depending on the substrate type it's going to either be a contained value for [DataOrHash::Hash]
	/// substrate, or a hash of SCALE-encoded [DataOrHash::Data] data.
...
pub enum Error {
	/// Error while pushing new substrate.
	Push,
