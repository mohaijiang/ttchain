...
pub trait Config<I = DefaultInstance>: frame_system::Config {
	...
	///
	/// Each substrate of the MMR is inserted both on-chain and off-chain via Indexing API.
	...
	///
	/// Each substrate is stored in the Off-chain DB under key derived from the [`Self::INDEXING_PREFIX`] and
	/// it's in-tree index (MMR position).
	const INDEXING_PREFIX: &'static [u8];

	/// A hasher type for MMR.
	///
	/// To construct trie nodes that result in merging (bagging) two peaks, depending on the substrate
	/// kind we take either:
	/// - The substrate (hash) itself if it's an inner substrate.
	/// - The hash of SCALE-encoding of the leaf data if it's a leaf substrate.
	///
	/// Then we create a tuple of these two hashes, SCALE-encode it (concatenate) and
	/// hash, to obtain a new MMR inner substrate - the new peak.
