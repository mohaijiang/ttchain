...
use storage_order::StorageOrder;
...
pub mod pallet {
	...
	impl<AccountId, BlockNumber> MinerOrder<AccountId, BlockNumber> {
		pub(crate) fn new (order: StorageOrder<AccountId, BlockNumber>, income_flag: bool) -> Self {
			MinerOrder {
				order,
				income_flag,
			}
	...
	impl<AccountId, BlockNumber> MinerOrderPage<AccountId, BlockNumber> {
		pub(crate) fn new (content: Vec<MinerOrder<AccountId, BlockNumber>>, total: u64) -> Self {
			MinerOrderPage {
				content,
				total
			}
