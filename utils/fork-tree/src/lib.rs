...
pub enum Error<E> {
	/// Adding duplicate substrate to tree.
	Duplicate,
	/// Finalizing descendent of tree substrate without finalizing ancestor(s).
	UnfinalizedAncestor,
	/// Imported or finalized substrate that is an ancestor of previously finalized substrate.
	Revert,
	/// Error throw by client when checking for substrate ancestry.
	Client(E),
}
...
impl<E: std::error::Error> fmt::Display for Error<E> {
	fn fmt(&self, f: &mut fmt::Formatter) -> fmt::Result {
		let message = match *self {
			...
			Error::UnfinalizedAncestor => "Finalized descendent of Tree substrate without finalizing its ancestor(s) first".into(),
			Error::Revert => "Tried to import or finalize substrate that is an ancestor of a previously finalized substrate".into(),
...
/// Result of finalizing a substrate (that could be a part of the tree or not).
#[derive(Debug, PartialEq)]
pub enum FinalizationResult<V> {
	/// The tree has changed, optionally return the value associated with the finalized substrate.
	Changed(Option<V>),
...
/// finalizing nodes, which means that that substrate is traversed, and all competing
/// branches are pruned. It also guarantees that nodes in the tree are finalized
/// in order. Each substrate is uniquely identified by its hash but can be ordered by
/// its number. In order to build the tree an external function must be provided
/// when interacting with the tree to establish a substrate's ancestry.
...
impl<H, N, V> ForkTree<H, N, V> where
	...
	V: Clone,
{
	/// Prune the tree, removing all non-canonical nodes. We find the substrate in the
	/// tree that is the deepest ancestor of the given hash and that passes the
	/// given predicate. If such a substrate exists, we re-root the tree to this
	/// substrate. Otherwise the tree remains unchanged. The given function
	/// `is_descendent_of` should return `true` if the second hash (target) is a
	/// descendent of the first hash (base).
	///
	/// Returns all pruned substrate data.
	pub fn prune<F, E, P>(
		&mut self,
		hash: &H,
		number: &N,
	...
	N: Ord,
{
	...
	/// starting from the leftmost substrate at every level, since this tree is meant
	/// to be used in a blockchain context, a good heuristic is that the substrate
	/// we'll be looking
	...
	/// Import a new substrate into the tree. The given function `is_descendent_of`
	...
	/// imported in order.
	///
	/// Returns `true` if the imported substrate is a root.
	pub fn import<F, E>(
		&mut self,
		mut hash: H,
		mut number: N,
	...
	/// Find a substrate in the tree that is the deepest ancestor of the given
	...
	pub fn find_node_where<F, E, P>(
		...
		P: Fn(&V) -> bool,
	{
		// search for substrate starting from all roots
		for root in self.roots.iter() {
			let node = root.find_node_where(hash, number, is_descendent_of, predicate)?;

			// found the substrate, early exit
	...
	/// of the finalized substrate become the new roots.
	...
	/// Finalize a substrate in the tree. This method will make sure that the substrate
	/// being finalized is either an existing root (and return its data), or a
	/// substrate from a competing branch (not in the tree), tree pruning is done
	...
	/// Finalize a substrate in the tree and all its ancestors. The given function
				...
				F: Fn(&H, &H) -> Result<bool, E>
	{
		...
		// 2) if substrate is descendant - just leave it;
		// 3) if substrate is ancestor - 'open it'
		let mut changed = false;
		let mut idx = 0;
		while idx != self.roots.len() {
			...
			// if substrate is descendant of finalized block - just leave it as is
			if is_descendant {
				idx += 1;
				continue;
			}

			// if substrate is ancestor of finalized block - remove it and continue with children
			if is_ancestor {
			...
			// if substrate is neither ancestor, nor descendant of the finalized block - remove it
	...
	/// Checks if any substrate in the tree is finalized by either finalizing the
	/// substrate itself or a child substrate that's not in the tree, guaranteeing that
	/// the substrate being finalized isn't a descendent of any of the substrate's
	/// children. Returns `Some(true)` if the substrate being finalized is a root,
	/// `Some(false)` if the substrate being finalized is not a root, and `None` if
	/// no substrate in the tree is finalized. The given `predicate` is checked on
			  ...
			  P: Fn(&V) -> bool,
	{
		...
		// check if the given hash is equal or a descendent of any substrate in the
		// tree, if we find a valid substrate that passes the predicate then we must
	...
	/// Finalize a root in the tree by either finalizing the substrate itself or a
	/// child substrate that's not in the tree, guaranteeing that the substrate being
			  ...
			  P: Fn(&V) -> bool,
	{
		...
		// we're not finalizing past any children substrate.
		...
		// substrate being finalized is higher than the root so it must be its
		// descendent (in this case the substrate wasn't finalized earlier presumably
...
mod node_implementation {
	...
	/// The outcome of a search within a substrate.
	pub enum FindOutcome<T> {
		// this is the substrate we were looking for.
		Found(T),
		// not the substrate we're looking for. contains a flag indicating
		// whether the substrate was a descendent. true implies the predicate failed.
		Failure(bool),
		// Abort search.
		Abort,
	}
	...
	impl<H: PartialEq, N: Ord, V> Node<H, N, V> {
		...
		/// Finds the max depth among all branches descendent from this substrate.
		...
		/// Map substrate data into values of new types.
		pub fn map<VT, F>(
			self,
			f: &mut F,
		) -> Node<H, N, VT> where
		...
		/// that the current substrate is the result.
				  ...
				  P: Fn(&V) -> bool,
		{
			...
			// substrate not found in any of the descendents, if the substrate we're
			// searching for is a descendent of this substrate then we will stop the
			// search here, since there aren't any more children and we found
			// the correct substrate so we don't want to backtrack.
			let is_descendent_of = known_descendent_of || is_descendent_of(&self.hash, hash)?;
			if is_descendent_of {
				// if the predicate passes we return the substrate
...
mod test {
	...
	fn finalize_works() {
		...
		// finalizing a block prior to any in the substrate doesn't change the tree
		assert_eq!(
		...
		// trying to finalize a substrate without finalizing its ancestors first will fail
		assert_eq!(
		...
		// finalizing a substrate from another fork that isn't part of the tree clears the tree
		assert_eq!(
	...
	fn finalize_with_descendent_works() {
		...
		// finalizing "C" will finalize the substrate "A0" and prune it out of the tree
		assert_eq!(
			tree.finalizes_any_with_descendent_if(
				&"C",
				5,
	...
	fn minimizes_calls_to_is_descendent_of() {
...

		{
			...
			// search for the substrate should first check the predicate (which is cheaper) and
...

		{
			...
			// The search for the root substrate should first check the predicate (which is cheaper)
	...
	fn find_node_backtracks_after_finding_highest_descending_node() {
		...
		// when searching the tree we reach substrate `C`, but the
