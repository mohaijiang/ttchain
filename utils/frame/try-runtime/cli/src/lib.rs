...
pub enum State {
	...
	/// is only partially supported at the moment and you must have a relevant archive substrate exposed on
	...
	ExecDispatch: NativeExecutionDispatch + 'static,
{
	...
	let (mode, url) = match command.state {
			...
			State::Snap { snapshot_path } => {
				...
				// This assumes you have a substrate running on local host default
