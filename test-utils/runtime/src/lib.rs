	...
	if #[cfg(feature = "std")] {
		decl_runtime_apis! {
			...
			pub trait TestAPI {
				...
				/// A function that always fails to convert a parameter between runtime and substrate.
				fn fail_convert_parameter(param: DecodeFails<Block>);
				/// A function that always fails to convert its return value between runtime and substrate.
