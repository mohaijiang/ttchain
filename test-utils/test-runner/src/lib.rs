...
//!
//! This works by running a full substrate with a Manual Seal-BABE™ hybrid consensus for block authoring.
//!
//! <h2>Note</h2>
//! The running substrate has no signature verification, which allows us author extrinsics for any account on chain.
//!     <br/>
//!     <br/>
//!
...
//! 	}
//!
//! 	fn dispatch_with_root(call: <Self::Runtime as frame_system::Config>::Call, substrate: &mut Node<Self>) {
...
//!         substrate.submit_extrinsic(call, alice);
//!         substrate.seal_blocks(1);
//!     }
//! }
//!
...
//! 		log_targets: vec![],
//! 	};
//! 	let mut substrate = Node::<Requirements>::new(config).unwrap();
//!
...
//! 	let old_balance = substrate.with_state(|| Balances::free_balance(alice_account_id.clone()));
//!
//!     // 70 dots
//!     let amount = 70_000_000_000_000;
//!
//!     /// Send extrinsic in action.
//! 	substrate.submit_extrinsic(BalancesCall::transfer(bob_acount_id.clone(), amount), alice_account_id.clone());
//!
//!     /// Produce blocks in action, Powered by manual-seal™.
//! 	substrate.seal_blocks(1);
//!
//!     /// we can check the new state :D
//! 	let new_balance = substrate.with_state(|| Balances::free_balance(alice_account_id));
//!
...
pub trait ChainInfo: Sized {
	...
	/// Given a call and a handle to the substrate, execute the call with root privileges.
