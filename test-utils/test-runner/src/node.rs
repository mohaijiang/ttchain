...
/// This holds a reference to a running substrate on another thread,
/// the substrate process is dropped when this struct is dropped
/// also holds logs from the process.
pub struct Node<T: ChainInfo> {
	/// rpc handler for communicating with the substrate over rpc.
	rpc_handler: Arc<MetaIoHandler<sc_rpc::Metadata, sc_rpc_server::RpcMiddleware>>,
	/// Stream of log lines
	log_stream: mpsc::UnboundedReceiver<String>,
	/// substrate tokio runtime
	_runtime: tokio::runtime::Runtime,
	/// handle to the running substrate.
...
/// Configuration options for the substrate.
...
impl<T: ChainInfo> Node<T> {
	/// Starts a substrate with the manual-seal authorship.
	...
	/// submit some extrinsic to the substrate, providing the sending account.
	...
	/// Checks the substrate logs for a specific entry.
	...
	/// Revert all blocks added since creation of the substrate.
	pub fn clean(&self) {
...
impl<T: ChainInfo> Drop for Node<T> {
	fn drop(&mut self) {
		...
		if let Some(mut task_manager) = self._task_manager.take() {
			// if this isn't called the substrate will live forever
