...
#[derive(StructOpt)]
struct Options {
	/// The path to the `substrate-template` source.
...
/// Copy the `substrate-template` to the given path.
fn copy_node_template(node_template: &Path, dest_path: &Path) {
	let options = CopyOptions::new();
	dir::copy(node_template, dest_path, &options).expect("Copies substrate-template to tmp dir");
}
	...
	let members = workspace_members.iter()
		.map(|p|
			p.strip_prefix(node_template_path)
				.expect("Workspace member is a child of the substrate template path!")
				.parent()
...
/// Build and test the generated substrate-template
...
fn main() {
	...
	// The path to the substrate-template in the build dir.
	...
	tar.append_dir_all("substrate-substrate-template", node_template_path)
		.expect("Writes substrate-substrate-template archive");
}
