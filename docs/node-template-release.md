...
the existence of your current git commit ID in the remote repository.
	...
	```bash
	cd .maintain/
	./substrate-template-release.sh <output tar.gz file>
	```
...
by running the following command.
	...
	cd substrate-substrate-template
	# rsync with force copying. Note the slash at the destination directory is important
	rsync -avh * <destination substrate-template directory>/
	# For dry-running add `-n` argument
	# rsync -avhn * <destination substrate-template directory>/
	```
