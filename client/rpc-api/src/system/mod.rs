...
pub trait SystemApi<Hash, Number> {
	/// Get the substrate's implementation name. Plain old string.
	#[rpc(name = "system_name")]
	fn system_name(&self) -> SystemResult<String>;

	/// Get the substrate implementation's version. Should be a semver string.
	...
	/// Return health status of the substrate.
	///
	...
	/// Returns the base58-encoded PeerId of the substrate.
	#[rpc(name = "system_localPeerId", returns = "String")]
	fn system_local_peer_id(&self) -> Receiver<String>;

	/// Returns the multiaddresses that the local substrate is listening on
	///
	...
	/// Returns the roles the substrate is running as.
	#[rpc(name = "system_nodeRoles", returns = "Vec<NodeRole>")]
	fn system_node_roles(&self) -> Receiver<Vec<NodeRole>>;

	/// Returns the state of the syncing of the substrate: starting block, current best block, highest
	/// known block.
