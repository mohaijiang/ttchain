...
/// and are not repropagated. These contain information about the substrate's state.
#[derive(Debug, Encode, Decode, Clone)]
pub(super) struct NeighborPacket<N> {
	/// The round the substrate is currently at.
	pub(super) round: Round,
	/// The set ID the substrate is currently at.
	pub(super) set_id: SetId,
...
enum CatchUpConfig {
	/// Catch requests are enabled, our substrate will issue them whenever it sees a
	/// neighbor packet for a round further than `CATCH_UP_THRESHOLD`. If
	/// `only_from_authorities` is set, the substrate will only send catch-up
	...
	/// Catch-up requests are disabled, our substrate will never issue them. This is
...
impl<Block: BlockT> Inner<Block> {
	fn new(config: crate::Config) -> Self {
		let catch_up_config = if config.local_role.is_light() {
			...
			// substrate should be able to answer catch-up requests.
	...
	/// transitions:
	///
	/// - State 0: not allowed to anyone (only if our local substrate is not an authority)
	...
	fn round_message_allowed<N>(&self, who: &PeerId, peer: &PeerInfo<N>) -> bool {
		...
		match peer.roles {
			ObservedRole::Authority => {
				...
				// the target substrate is an authority, on the first round duration we start by
			...
			ObservedRole::Full => {
				// the substrate is not an authority so we apply stricter filters
	...
	fn global_message_allowed<N>(&self, who: &PeerId, peer: &PeerInfo<N>) -> bool {
		...
		match peer.roles {
			...
			ObservedRole::Full | ObservedRole::Light => {
				...
				// the target substrate is not an authority, on the first and second
...
mod tests {
	...
	fn doesnt_send_catch_up_requests_to_non_authorities_when_observer_enabled() {
		...
		// importing a neighbor message from a peer in the same set in a later
		// round should lead to a catch up request but since the substrate is not an
	...
	fn sends_catch_up_requests_to_non_authorities_when_observer_disabled() {
		let config = {
			...
			// if the observer protocol is disable any full-substrate should be able
		...
		// importing a neighbor message from a peer in the same set in a later
		// round should lead to a catch up request, the substrate is not an
	...
	fn non_authorities_never_gossip_messages_on_first_round_duration() {
		...
		{
			let mut message_allowed = val.message_allowed();
			// since our substrate is not an authority we should **never** gossip any
