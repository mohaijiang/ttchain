	...
	futures::executor::block_on(futures::future::poll_fn(move |cx| {
		...
		for _ in 0..2500 {
				...
				.sample(&mut rng)
			{
				...
				// If we generate 1, discover a new substrate.
				1 => {
				...
				// If we generate 3, disconnect from a random substrate.
				3 => {
				...
				// If we generate 4, connect to a random substrate.
				4 => {
				...
				// 7 and 8 are about switching a random substrate in or out of reserved mode.
				7 => {
					if let Some(id) = known_nodes
						.iter()
