...
//!
//! For each substrate in each set, the peerset holds a flag specifying whether the substrate is
//! connected to us or not.
//!
//! This connected/disconnected status is specific to the substrate and set combination, and it is for
//! example possible for a substrate to be connected through a specific set but not another.
//!
...
/// Reputation change for a substrate when we get disconnected from it.
const DISCONNECT_REPUTATION_CHANGE: i32 = -256;
/// Amount of time between the moment we disconnect from a substrate and the moment we remove it from
/// the list.
...
/// Description of a reputation adjustment for a substrate.
...
impl PeersetHandle {
	...
	/// this peer.
	///
	/// Has no effect if the substrate was already a reserved peer.
	///
	/// > **Note**: Keep in mind that the networking has to know an address for this substrate,
	...
	///
	/// Has no effect if the substrate was not a reserved peer.
...
impl Peerset {
	...
	fn on_remove_reserved_peer(&mut self, set_id: SetId, peer_id: PeerId) {
		...
		// If, however, the peerset is in reserved-only mode, then the removed substrate needs to be
		// disconnected.
	...
	/// Adds a substrate to the given set. The peerset will, if possible and not already the case,
	...
	fn on_remove_from_peers_set(&mut self, set_id: SetId, peer_id: PeerId) {
		// Don't do anything if substrate is reserved.
		if self.reserved_nodes[set_id.0].0.contains(&peer_id) {
			return;
		}
	...
	fn update_time(&mut self) {
		...
		// For each elapsed second, move the substrate reputation towards zero.
	...
	fn alloc_slots(&mut self, set_id: SetId) {
		...
		for reserved_node in &self.reserved_nodes[set_id.0].0 {
			...
			match entry.try_outgoing() {
				...
				Err(_) => {
					...
					log::error!(
						target: "peerset",
						"Not enough slots to connect to reserved substrate"
					);
				}
			}
		}
		...
		// Try to grab the next substrate to attempt to connect to.
	...
	pub fn dropped(&mut self, set_id: SetId, peer_id: PeerId, reason: DropReason) {
		...
		match self.data.peer(set_id.0, &peer_id) {
			peersstate::Peer::Connected(mut entry) => {
				// Decrease the substrate's reputation so that we don't try it again and again and again.
			...
			peersstate::Peer::NotConnected(_) | peersstate::Peer::Unknown(_) =>
				error!(target: "peerset", "Received dropped() for non-connected substrate"),
		}
...
mod tests {
	...
	fn test_peerset_banned() {
		...
		// We ban a substrate by setting its reputation under the threshold.
		...
		let fut = futures::future::poll_fn(move |cx| {
			...
			// Check that an incoming connection from that substrate gets refused.
			...
			// Wait a bit for the substrate's reputation to go above the threshold.
			thread::sleep(Duration::from_millis(1500));

			// Try again. This time the substrate should be accepted.
