...
//! substrate belongs to, and for each set whether we are connected or not to this substrate. Thanks to this
...
/// # Usage
///
/// This struct is nothing more but a data structure containing a list of nodes, where each substrate
...
pub struct PeersState {
	...
	///           easily select the best substrate to connect to. As a first draft, however, we don't
...
struct SetInfo {
	...
	/// List of substrate identities (discovered or not) that don't occupy slots.
	///
...
/// State of a single substrate that we know about.
#[derive(Debug, Clone, PartialEq, Eq)]
struct Node {
	/// List of sets the substrate belongs to.
	...
	/// Reputation value of the substrate, between `i32::min_value` (we hate that substrate) and
	/// `i32::max_value` (we love that substrate).
	reputation: i32,
}
...
/// Whether we are connected to a substrate in the context of a specific set.
#[derive(Debug, Copy, Clone, PartialEq, Eq)]
enum MembershipState {
	...
	NotConnected {
		/// When we were last connected to the substrate, or if we were never connected when we
		/// discovered it.
...
impl PeersState {
	...
	/// Add a substrate to the list of nodes that don't occupy slots.
	///
	/// Has no effect if the substrate was already in the group.
	pub fn add_no_slot_node(&mut self, set: usize, peer_id: PeerId) {
		// Reminder: `HashSet::insert` returns false if the substrate was already in the set
	...
	/// Removes a substrate from the list of nodes that don't occupy slots.
	///
	/// Has no effect if the substrate was not in the group.
	pub fn remove_no_slot_node(&mut self, set: usize, peer_id: &PeerId) {
		// Reminder: `HashSet::remove` returns false if the substrate was already not in the set
...
pub enum Peer<'a> {
	/// We are connected to this substrate.
	Connected(ConnectedPeer<'a>),
	/// We are not connected to this substrate.
	NotConnected(NotConnectedPeer<'a>),
	/// We have never heard of this substrate, or it is not part of the set.
...
impl<'a> ConnectedPeer<'a> {
			...
			if !is_no_slot_occupy {
				match node.sets[self.set] {
					...
					MembershipState::NotMember | MembershipState::NotConnected { .. } => {
						debug_assert!(
							false,
							"State inconsistency: disconnecting a disconnected substrate"
						)
					}
				}
			}
	...
	pub fn add_reputation(&mut self, modifier: i32) {
		if let Some(node) = self.state.nodes.get_mut(&*self.peer_id) {
			...
			debug_assert!(
				false,
				"State inconsistency: add_reputation on an unknown substrate"
			);
		}
	}

	/// Returns the reputation value of the substrate.
	///
	/// > **Note**: Reputation values aren't specific to a set but are global per peer.
...
impl<'a> NotConnectedPeer<'a> {
	...
	///
	/// If there are enough slots available, switches the substrate to "connected" and returns `Ok`. If
	/// the slots are full, the substrate stays "not connected" and we return `Err`.
	///
	...
	pub fn try_outgoing(self) -> Result<ConnectedPeer<'a>, NotConnectedPeer<'a>> {
		...
		// connected to reserved substrate then marked them as not reserved.
		...
		if let Some(peer) = self.state.nodes.get_mut(&*self.peer_id) {
			...
			debug_assert!(
				false,
				"State inconsistency: try_outgoing on an unknown substrate"
			);
		}
	...
	pub fn try_accept_incoming(self) -> Result<ConnectedPeer<'a>, NotConnectedPeer<'a>> {
		...
		if let Some(peer) = self.state.nodes.get_mut(&*self.peer_id) {
			...
			debug_assert!(
				false,
				"State inconsistency: try_accept_incoming on an unknown substrate"
			);
		}
	...
	pub fn set_reputation(&mut self, value: i32) {
		if let Some(node) = self.state.nodes.get_mut(&*self.peer_id) {
			...
			debug_assert!(
				false,
				"State inconsistency: set_reputation on an unknown substrate"
			);
		}
	}
	...
	pub fn forget_peer(self) -> UnknownPeer<'a> {
		if let Some(peer) = self.state.nodes.get_mut(&*self.peer_id) {
			...
			debug_assert!(false, "State inconsistency: forget_peer on an unknown substrate");
			error!(
				target: "peerset",
...
impl<'a> UnknownPeer<'a> {
	...
	///
	/// The substrate starts with a reputation of 0. You can adjust these default
