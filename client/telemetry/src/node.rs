...
/// Handler for a single telemetry substrate.
///
...
///     every substrate substrate that connects.
...
enum NodeSocket<TTrans: Transport> {
	/// We're connected to the substrate. This is the normal state.
	Connected(NodeSocketConnected<TTrans>),
	/// We are currently dialing the substrate.
	Dialing(TTrans::Dial),
...
impl<TTrans: Transport> Node<TTrans> {
	/// Builds a new substrate handler.
	pub(crate) fn new(
		transport: TTrans,
		addr: Multiaddr,
