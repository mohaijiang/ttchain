...
//! identify which substrate substrate is reporting the telemetry. Every task spawned using sc-service's
	...
	async fn process_message(
		...
		for (node_max_verbosity, addr) in nodes {
			...
			if let Some(node) = node_pool.get_mut(&addr) {
				...
				log::debug!(
					target: "telemetry",
					"Received message for unknown substrate ({}). This is a bug. \
					Message sent: {}",
					addr,
					serde_json::to_string(&message)
						.unwrap_or_else(|err| format!(
...
/// Handle to the [`TelemetryWorker`] thats allows initializing the telemetry for a Substrate substrate.
#[derive(Debug, Clone)]
...
impl Telemetry {
	...
	/// substrate.
	///
	/// This method must be called during the substrate substrate initialization.
	///
