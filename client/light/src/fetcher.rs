...
impl<E, H, B: BlockT, S: BlockchainStorage<B>> LightDataChecker<E, H, B, S> {
			...
			H::Out: Ord + codec::Codec,
	{
		...
		// => remote substrate can't use max block greater that one that we have passed
		if remote_proof.max_block > request.max_block.0 || remote_proof.max_block < request.last_block.0 {
			return Err(ClientError::ChangesTrieAccessFailed(format!(
				"Invalid max_block used by the remote substrate: {}. Local: {}..{}..{}",
				remote_proof.max_block, request.first_block.0, request.last_block.0, request.max_block.0,
			)).into());
		}

		// check if remote substrate has responded with extra changes trie roots proofs
		...
		if is_extra_first_root || is_extra_last_root {
			return Err(ClientError::ChangesTrieAccessFailed(format!(
				"Extra changes tries roots proofs provided by the remote substrate: [{:?}..{:?}]. Expected in range: [{}; {})",
		...
		// => remote substrate has sent us CHT-based proof of required changes tries roots
