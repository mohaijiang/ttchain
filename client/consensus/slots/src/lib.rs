	...
	async fn on_slot(
		...
		let proposal = match futures::future::select(proposing, proposing_remaining).await {
			...
			Either::Right(_) => {
				...
				// If the substrate was compiled with debug, tell the user to use release optimizations.
				#[cfg(build_type="debug")]
				info!(
					target: logging_target,
					"👉 Recompile your substrate in `--release` mode to mitigate this problem.",
				);
				telemetry!(
					telemetry;
					CONSENSUS_INFO;
	...
	loop {
			...
			.can_author_with(&BlockId::Hash(slot_info.chain_head.hash()))
		{
			warn!(
				...
				Probably a substrate update is required!",
				slot_info.slot,
				err,
			);
		} else {
