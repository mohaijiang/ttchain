	...
	Number: Ord + One + Zero + Add<Output=Number> + Copy,
{
	...
	pub fn epoch_descriptor_for_child_of<D: IsDescendentOfBuilder<Hash>>(
		...
		// find_node_where will give you the substrate in the fork-tree which is an ancestor
		...
		// We want to find the deepest substrate in the tree which is an ancestor
		...
		// at epoch_1 -- all we're doing here is figuring out which substrate
		// we need.
		let predicate = |epoch: &PersistedEpochHeader<E>| match *epoch {
		...
		self.inner.find_node_where(
			...
			.map(|n| {
				n.map(|node| (match node.data {
					// Ok, we found our substrate.
					// and here we figure out which of the internal epochs
					// of a genesis substrate to use based on their start slot.
