...
/// The maximum number of characters for a substrate name.
...
pub trait CliConfiguration<DCV: DefaultConfigurationValues = ()>: Sized {
	...
	/// Returns `true` if the substrate is for development or not
	///
	/// By default this is retrieved from `SharedParams`.
	...
	/// Get the substrate key from the current object
	///
...
/// Generate a valid random name for the substrate
