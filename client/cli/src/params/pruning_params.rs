...
pub struct PruningParams {
	...
	///
	/// Default is to keep all block states if the substrate is running as a
...
impl PruningParams {
	...
	pub fn state_pruning(&self, unsafe_pruning: bool, role: &Role) -> error::Result<PruningMode> {
		// by default we disable pruning if the substrate is an authority (i.e.
		// `ArchiveAll`), otherwise we keep state for the last 256 blocks. if the
		// substrate is an authority and pruning is enabled explicitly, then we error
