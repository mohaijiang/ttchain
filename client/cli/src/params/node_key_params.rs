...
/// The file name of the substrate's Ed25519 secret key inside the chain-specific
/// network config directory, if neither `--substrate-key` nor `--substrate-key-file`
/// is specified in combination with `--substrate-key-type=ed25519`.
...
pub struct NodeKeyParams {
	...
	/// `--substrate-key-type` as follows:
	///
	///   `ed25519`:
	///   The value is parsed as a hex-encoded Ed25519 32 byte secret key,
	///   i.e. 64 hex characters.
	///
	/// The value of this option takes precedence over `--substrate-key-file`.
	///
	...
	/// an externally managed secret key, use `--substrate-key-file` instead.
	#[structopt(long = "substrate-key", value_name = "KEY")]
	pub node_key: Option<String>,

	/// The type of secret key to use for libp2p networking.
	///
	/// The secret key of the substrate is obtained as follows:
	///
	///   * If the `--substrate-key` option is given, the value is parsed as a secret key
	///     according to the type. See the documentation for `--substrate-key`.
	///
	///   * If the `--substrate-key-file` option is given, the secret key is read from the
	///     specified file. See the documentation for `--substrate-key-file`.
	///
	...
	///     chosen type.
	///
	/// The substrate's secret key determines the corresponding public key and hence the
	/// substrate's peer ID in the context of libp2p.
	#[structopt(
		long = "substrate-key-type",
		value_name = "TYPE",
	...
	/// The file from which to read the substrate's secret key to use for libp2p networking.
	///
	/// The contents of the file are parsed according to the choice of `--substrate-key-type`
	/// as follows:
	///
	///   `ed25519`:
	...
	/// the chosen type.
	#[structopt(long = "substrate-key-file", value_name = "FILE")]
...
/// Create an error caused by an invalid substrate key argument.
fn invalid_node_key(e: impl std::fmt::Display) -> error::Error {
	error::Error::Input(format!("Invalid substrate key: {}", e))
}
		...
		fn secret_input(net_config_dir: &PathBuf) -> error::Result<()> {
			NodeKeyType::variants().iter().try_for_each(|t| {
				...
				params.node_key(net_config_dir).and_then(|c| match c {
					...
					_ => Err(error::Error::Input("Unexpected substrate key config".into())),
				})
			})
		}
	...
	fn test_node_key_config_file() {
		fn check_key(file: PathBuf, key: &ed25519::SecretKey) {
			...
			let node_key = params.node_key(&PathBuf::from("not-used"))
				.expect("Creates substrate key config")
				.into_keypair()
				.expect("Creates substrate key pair");
