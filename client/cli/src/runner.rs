...
/// A Substrate CLI runtime that can be used to run a substrate or a command
...
impl<C: SubstrateCli> Runner<C> {
	...
	/// Log information about the substrate itself.
	///
	/// # Example:
	///
	/// ```text
	...
	/// 2020-06-03 16:14:21 ⛓  Native runtime: substrate-251 (substrate-substrate-1.tx1.au10)
	/// ```
	...
	/// A helper function that runs a substrate with tokio and stops if the process receives the signal
	/// `SIGTERM` or `SIGINT`.
	...
	/// A helper function that runs a command with the configuration of this substrate.
	pub fn sync_run<E>(
		self,
	...
	/// Get an immutable reference to the substrate Configuration
	pub fn config(&self) -> &Configuration {
		&self.config
	}

	/// Get a mutable reference to the substrate Configuration
