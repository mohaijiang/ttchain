...
/// The `run` command used to run a substrate.
#[derive(Debug, StructOpt, Clone)]
pub struct RunCmd {
	/// Enable validator mode.
	///
	/// The substrate will be started with the authority role and actively
	...
	/// The human-readable name for this substrate.
	///
	/// The substrate name will be reported to the telemetry server, if enabled.
	...
	/// Run a temporary substrate.
	///
	/// A temporary directory will be created to store the configuration and will be deleted
	/// at the end of the process.
	///
	/// Note: the directory is random per process execution. This directory is used as base path
	/// which includes: database, substrate key and keystore.
...
impl CliConfiguration for RunCmd {
	...
	fn node_name(&self) -> Result<String> {
		...
		is_node_name_valid(&name).map_err(|msg| {
			Error::Input(format!(
				"Invalid substrate name '{}'. Reason: {}. If unsure, use none.",
				name, msg
		))
		})?;
...
/// Check whether a substrate name is considered as valid.
...
fn rpc_interface(
	...
	if is_external && is_validator && rpc_methods != RpcMethods::Unsafe {
		...
		used if the substrate is running as a validator. Use `--unsafe-rpc-external` \
