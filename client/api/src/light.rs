...
pub struct RemoteChangesRequest<Header: HeaderT> {
	...
	/// Proofs for roots of ascendants of tries_roots.0 are provided by the remote substrate.
...
pub struct ChangesProof<Header: HeaderT> {
	...
	/// the requester' substrate. It is a map of block number => changes trie root.
	pub roots: BTreeMap<Header::Number, Header::Hash>,
	/// The proofs for all changes tries roots that have been touched AND are
	/// missing from the requester' substrate. It is a map of CHT number => proof.
...
pub enum LocalOrRemote<Data, Request> {
	...
	Local(Data),
	/// When data is unavailable locally, the request to fetch it from remote substrate is returned.
	Remote(Request),
...
/// locally, or fetches required data from remote substrate.
...
pub mod tests {
	...
	#[error("Not implemented on test substrate")]
	struct MockError;
