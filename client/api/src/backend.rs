...
pub trait Backend<Block: BlockT>: AuxStore + Send + Sync {
	...
	/// revert past any finalized block, this is unsafe and can potentially leave the substrate in an
	/// inconsistent state.
	///
...
pub trait RemoteBackend<Block: BlockT>: Backend<Block> {
	...
	/// locally, or prepares request to fetch that data from remote substrate.
