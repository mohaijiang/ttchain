...
/// An authority discovery [`Worker`] can publish the local substrate's addresses as well as discover
...
///
///    2. Get the list of keys owned by the local substrate participating in the current authority set.
///
	...
	DhtEventStream: Stream<Item = DhtEvent> + Unpin,
{
	...
	pub(crate) fn new(
		...
		// When a substrate starts up publishing and querying might fail due to various reasons, for
		// example due to being not yet fully bootstrapped on the DHT. Thus one should retry rather
		// sooner than later. On the other hand, a long running substrate is likely well connected and
	...
	//
	// A substrate might have multiple authority discovery keys within its keystore, e.g. an old one and
