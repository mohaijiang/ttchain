...
pub struct WorkerConfig {
	/// The maximum interval in which the substrate will publish its own address on the DHT.
	///
	...
	/// The maximum interval in which the substrate will query the DHT for new entries.
	///
	/// By default this is set to 10 minutes.
	pub max_query_interval: Duration,

	/// If `false`, the substrate won't publish on the DHT multiaddresses that contain non-global
...
impl Default for WorkerConfig {
	fn default() -> Self {
		Self {
			...
			// 24h through libp2p-kad. Given that a substrate could restart at any point in time, one can
