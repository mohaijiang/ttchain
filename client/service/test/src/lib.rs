...
{
	let root = root.path().join(format!("substrate-{}", index));
	...
	G: RuntimeGenesis + 'static,
{
	...
	fn insert_nodes(
		...
		for (key, authority) in authorities {
			...
			let (service, user_data) = authority(node_config).expect("Error creating test substrate service");
		...
		for full in full {
			...
			let addr = node_config.network.listen_addresses.iter().next().unwrap().clone();
			let (service, user_data) = full(node_config).expect("Error creating test substrate service");
		...
		for light in light {
			...
			let addr = node_config.network.listen_addresses.iter().next().unwrap().clone();
			let service = light(node_config).expect("Error creating test substrate service");
