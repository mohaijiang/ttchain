...
fn sync_cycle_from_offline_to_syncing_to_offline() {
	...
	// Now drop nodes 1 and 2, and check that substrate 0 is offline.
	net.peers.remove(2);
	net.peers.remove(1);
	block_on(futures::future::poll_fn::<(), _>(|cx| {
		net.poll(cx);
...
fn blocks_are_not_announced_by_light_nodes() {
	...
	// Add another substrate and remove substrate 0.
	net.add_full_peer();
	net.peers.remove(0);
...
fn can_not_sync_from_light_peer() {
	...
	// given the network with 1 full nodes (#0) and 1 light substrate (#1)
	...
	// and let the light client sync from this substrate
	net.block_until_sync();
...
/// When we don't inform the sync protocol about the best block, a substrate will not sync from us as the
...
fn sync_to_tip_requires_that_sync_protocol_is_informed_about_best_block() {
	...
	// Add a substrate and wait until they are connected
	net.add_full_peer_with_config(Default::default());
	net.block_until_connected();
	net.block_until_idle();
	...
	// Connect another substrate that should now sync to the tip
	net.add_full_peer_with_config(Default::default());
	net.block_until_connected();
...
/// Ensures that if we as a syncing substrate sync to the tip while we are connected to another peer
