...
mod rep {
	...
	///
	/// This forces substrate to verify it, thus the negative value here. Once transaction is verified,
...
impl<B: BlockT + 'static, H: ExHashT> TransactionsHandler<B, H> {
	...
	fn on_transactions(
		...
		// sending transaction to light substrate is considered a bad behavior
		if matches!(self.local_role, config::Role::Light) {
			debug!(target: "sync", "Peer {} is trying to send transactions to the light substrate", who);
	...
	fn do_propagate_transactions(
		...
		for (who, peer) in self.peers.iter_mut() {
			// never send transactions to the light substrate
