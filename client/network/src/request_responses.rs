...
pub struct ProtocolConfig {
    ...
    ///
    /// Every time a peer sends a request to the local substrate using this protocol, the networking
    ...
    /// the channel being full is an indicator that the substrate is overloaded.
    ///
    ...
    /// build a response.
    ///
    /// Can be `None` if the local substrate does not support answering incoming requests.
    /// If this is `None`, then the local substrate will not advertise support for this protocol towards
    /// other peers. If this is `Some` but the channel is closed, then the local substrate will
				...
				if pending_response.send(Err(RequestFailure::NotConnected)).is_err() {
					log::debug!(
						...
						"Not connected to peer {:?}. At the same time local \
						 substrate is no longer interested in the result.",
						target,
					);
				};
			}
...
pub enum RequestFailure {
	...
	Refused,
	/// The remote replied, but the local substrate is no longer interested in the response.
	Obsolete,
