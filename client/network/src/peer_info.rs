...
/// Time after we disconnect from a substrate before we purge its information from the cache.
const CACHE_EXPIRE: Duration = Duration::from_secs(10 * 60);
/// Interval at which we perform garbage collection on the substrate info.
...
/// Information about a substrate we're connected to.
#[derive(Debug)]
struct NodeInfo {
	/// When we will remove the entry about this substrate from the list, or `None` if we're connected
	/// to the substrate.
	...
	/// Latest ping time with this substrate.
...
impl PeerInfoBehaviour {
	...
	/// Borrows `self` and returns a struct giving access to the information about a substrate.
	///
	/// Returns `None` if we don't know anything about this substrate. Always returns `Some` for nodes
	...
	/// Inserts a ping time in the cache. Has no effect if we don't have any entry for that substrate,
	...
	fn handle_ping_report(&mut self, peer_id: &PeerId, ping_time: Duration) {
		...
		if let Some(entry) = self.nodes_info.get_mut(peer_id) {
			...
			error!(target: "sub-libp2p",
				"Received ping from substrate we're not connected to {:?}", peer_id);
		}
	}

	/// Inserts an identify record in the cache. Has no effect if we don't have any entry for that
	/// substrate, which shouldn't happen.
	fn handle_identify_report(&mut self, peer_id: &PeerId, info: &IdentifyInfo) {
		...
		if let Some(entry) = self.nodes_info.get_mut(peer_id) {
			...
			error!(target: "sub-libp2p",
				"Received pong from substrate we're not connected to {:?}", peer_id);
		}
	}
}

/// Gives access to the information about a substrate.
pub struct Node<'a>(&'a NodeInfo);

impl<'a> Node<'a> {
	/// Returns the endpoint of an established connection to the peer.
	///
	/// Returns `None` if we are disconnected from the substrate.
	...
	/// Returns the latest ping time we know of for this substrate. `None` if we never successfully
	/// pinged this substrate.
...
impl NetworkBehaviour for PeerInfoBehaviour {
	...
	fn inject_disconnected(&mut self, peer_id: &PeerId) {
		...
		if let Some(entry) = self.nodes_info.get_mut(peer_id) {
			...
			error!(target: "sub-libp2p",
				"Disconnected from substrate we were not connected to {:?}", peer_id);
		}
	}
