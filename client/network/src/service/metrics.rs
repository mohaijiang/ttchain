	...
	fn register(registry: &Registry) -> Result<Self, PrometheusError> {
		Ok(Self {
			...
			issued_light_requests: prometheus::register(Counter::new(
				...
				"Number of light client requests that our substrate has issued.",
			)?, registry)?,
			...
			requests_in_failure_total: prometheus::register(CounterVec::new(
				Opts::new(
					...
					"Total number of incoming requests that the substrate has failed to answer"
				),
				&["protocol", "reason"]
			)?, registry)?,
	...
	fn register(registry: &Registry, value: Arc<AtomicBool>) -> Result<(), PrometheusError> {
		prometheus::register(SourcedGauge::new(
			&Opts::new(
				...
				"Whether the substrate is performing a major sync or not.",
			),
