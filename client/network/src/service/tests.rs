...
/// Builds a full substrate to be used for testing. Returns the substrate service and its associated events
/// stream.
///
...
fn notifications_back_pressure() {
	// Node 1 floods substrate 2 with notifications. Random sleeps are done on substrate 2 to simulate the
	// substrate being busy. We make sure that all notifications are received.
...
fn ensure_listen_addresses_consistent_with_transport_memory() {
	...
	let _ = build_test_full_node(config::NetworkConfiguration {
		listen_addresses: vec![listen_addr.clone()],
		transport: config::TransportConfig::MemoryOnly,
		.. config::NetworkConfiguration::new("test-substrate", "test-client", Default::default(), None)
	});
}
