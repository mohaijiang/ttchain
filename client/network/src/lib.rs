...
//!
//! In a decentralized network, each substrate possesses a network private key and a network public key.
//! In Substrate, the keys are based on the ed25519 curve.
//!
//! From a substrate's public key, we can derive its *identity*. In Substrate and libp2p, a substrate's
...
//! be faked**.
//!
//! A substrate's identity uniquely identifies a machine on the network. If you start two or more
...
//!
//! In order for our substrate to join a peer-to-peer network, it has to know a list of nodes that are
...
//! Substrate uses:
//!
//! - Bootstrap nodes. These are hard-coded substrate identities and addresses passed alongside with
...
//!
//! When substrate Alice knows substrate Bob's identity and address, it can establish a connection with Bob.
//! All connections must always use encryption and multiplexing. While some substrate addresses (eg.
...
//! substrate's address can contain a domain name.
...
//! that we should disconnect a substrate, then that substream is closed.
//!
...
//! - Light-client requests. When a light client requires information, a random substrate we have a
...
//! substrate which initiated the substream can push notifications. If the remote wants to send
...
//! `sc-network` automatically tries to open a substream towards each substrate for which the legacy
...
//! integer representing the role of the substrate:
//!
//! - 1 for a full substrate.
//! - 2 for a light substrate.
//! - 4 for an authority.
//!
