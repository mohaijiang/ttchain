...
//!
//! At any given point in time, a running substrate tries as much as possible to be at the head of the
...
pub struct ChainSync<B: BlockT> {
	...
	/// The role of this substrate, e.g. light or full
	role: Roles,
	/// What block attributes we require for this substrate, usually derived from
...
enum PreValidateBlockAnnounce<H> {
	...
	///
	/// An error means that *this* substrate failed to validate it because some internal error happened.
...
impl<B: BlockT> ChainSync<B> {
	...
	{
		// There is nothing sync can get from the substrate that has no blockchain data.
...
mod test {
	...
	///
	/// The substrate is connected to multiple peers. Both of these peers are having a best block (1) that
	...
	///
	/// The scenario is that the substrate is doing a full resync and is connected to some substrate that is
	/// doing a major sync as well. This other substrate that is doing a major sync will finish before
	/// our substrate and send a block announcement message, but we don't have seen any block announcement
	/// from this substrate in its sync process. Meaning our common number didn't change. It is now expected
	...
	fn do_ancestor_search_when_common_block_to_best_qeued_gap_is_to_big() {
		...
		// Connect the substrate we will sync from
