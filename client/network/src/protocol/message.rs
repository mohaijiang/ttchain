...
pub mod generic {
	...
	bitflags! {
		/// Bitmask of the roles that a substrate fulfills.
		pub struct Roles: u8 {
			/// No network.
			const NONE = 0b00000000;
			/// Full substrate, does not participate in consensus.
	...
	pub struct RemoteChangesRequest<H> {
		...
		pub max: H,
		/// Storage child substrate key which changes are requested.
	...
	pub struct RemoteChangesResponse<N, H> {
		...
		/// Changes tries roots missing on the requester' substrate.
		pub roots: Vec<(N, H)>,
