...
pub enum Event {
	...
	/// Opened a substream with the given substrate with the given notifications protocol.
	///
	...
	/// Closed a substream with the given substrate. Always matches a corresponding previous
	...
	/// Received one or more messages from the given substrate using the given protocol.
...
/// Role that the peer sent to us during the handshake, with the addition of what our local substrate
/// knows about that peer.
///
/// > **Note**: This enum is different from the `Role` enum. The `Role` enum indicates what a
/// >			substrate says about itself, while `ObservedRole` is a `Role` merged with the
/// >			information known locally about that substrate.
#[derive(Debug, Clone)]
