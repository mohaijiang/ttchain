...
//!
//! - Bootstrap nodes. These are hard-coded substrate identities and addresses passed in the constructor
...
//! `DiscoveryOut::Discovered` that is generated whenever we discover a substrate.
//! Only the identity of the substrate is returned. The substrate's addresses are stored within the
...
//! of a substrate's address, you must call `add_self_reported_address`.
//!
...
pub struct DiscoveryBehaviour {
	...
	/// Identity of our local substrate.
	local_peer_id: PeerId,
...
impl DiscoveryBehaviour {
	...
	pub fn add_self_reported_address(
		...
		if !added {
			log::trace!(
				target: "sub-libp2p",
				"Ignoring self-reported address {} from {} as remote substrate is not part of any \
				 Kademlia DHTs supported by the local substrate.", addr, peer_id,
			);
		}
	}
...
pub enum DiscoveryOut {
	...
	/// A peer connected to this substrate for whom no listen address is known.
	///
	...
	fn inject_new_external_addr(&mut self, addr: &Multiaddr) {
		...
		if self.known_external_addresses.insert(new_addr.clone()) {
			info!(target: "sub-libp2p",
				"🔍 Discovered new external address for our substrate: {}",
				new_addr,
			);
		}
